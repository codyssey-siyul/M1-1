...
가동률지수는 분석 목적에 따라 원지수와 계절조정지수를 구분하여 사용하였다.  
월별 계절성을 확인할 때는 계절적 특성이 포함된 원지수를 사용하고, 계절적 영향을 줄인 상태에서 가동률의 변화를 비교할 때는 계절조정지수를 사용하였다.  
...
| 분석 방법 | 적용 기준 | 활용 목적 |
|:---:|:---:|:---:|
| **12개월 이동평균** | 최근 12개월 평균 | 생산능력지수의 단기 변동을 완화하고 장기 추세 확인 |
| **전월 대비 변화율** | 이전 달 대비 증감률(%) | 계절조정 가동률지수의 주요 상승·하락 시점 확인 |
| **연도별 표준편차** | 연도별 월별 가동률의 표준편차 | 연도별 가동률 변동성 비교 |
| **월별 평균** | 2015~2025년 완결 연도 사용 | 가동률 원지수의 월별 계절 패턴 확인 |
| **원지수·계절조정지수 차이** | `원지수 - 계절조정지수` | 계절조정 전후의 차이가 큰 시점 확인 |
| **시계열 분해** | 가법 모형, 주기 12개월, 2015~2025년 | 추세·계절성·잔차를 분리하여 계절 패턴 추가 확인 |
...
가동률의 추세와 계절적 패턴을 보다 명확하게 확인하기 위해 2015~2025년 완결 연도의 가동률 원지수를 대상으로 **시계열 분해(Time Series Decomposition)** 를 추가로 수행하였다.

월별 데이터의 특성을 고려하여 12개월 주기의 가법 모형(Additive Model)을 적용하고, 시계열을 관측값(Observed), 추세(Trend), 계절성(Seasonal), 잔차(Residual)로 분해하였다.

![가동률지수 시계열 분해](images/05_utilization_decomposition.png)

분해 결과 장기적인 가동률 추세와 함께 반복적인 계절적 패턴이 나타났다. 계절 성분은 **3월 +9.829로 가장 높았고, 8월 -15.863으로 가장 낮았다.**

이는 앞선 월별 평균 분석에서 **3월의 평균 가동률이 가장 높고 8월이 가장 낮게 나타난 결과와 일치**하며, 월별 가동률 차이가 반복적인 계절 패턴으로 해석될 수 있다는 근거를 보강한다.

잔차에서는 일부 시기에 비교적 큰 변동이 나타났으며, 이는 추세와 반복적인 계절성만으로 설명되지 않는 단기적인 변동이 존재했음을 보여준다.
...
- 시계열 분해의 월별 계절 성분을 기존 월별 평균 분석 결과와 비교하여 일치 여부 확인
